...
@Component({
  selector: 'app-admin-analytics-liveview',
  templateUrl: './admin-analytics-liveview.component.html',
  styleUrls: ['./admin-analytics-liveview.component.css']
})
export class AdminAnalyticsLiveviewComponent implements OnInit {
  ...
  constructor(public adminhttp: AdminHttpService, public toastr: ToastrService) {
  }

  getLivedata() {
    var data = ''
     this.adminhttp.PostAPI('admin/live-view', data).then((resdata: any) => {
       if (resdata.status == 200) {
         this.toastr.success(resdata.message);
         this.total_sale = resdata['data'][0][0]['total_sale'];
         this.total_order = resdata['data'][1][0]['total_order'];
         this.checking_out = resdata['data'][2][0]['checking_out'];
         this.purchased = resdata['data'][3][0]['purchased'];
         this.total_cart = resdata['data'][4][0]['total_cart'];
       } else {
  ...
  ngOnInit(): void {
    ...
    chart.backgroundSeries.mapPolygons.template.polygon.fill = am4core.color("#ffffff00");
