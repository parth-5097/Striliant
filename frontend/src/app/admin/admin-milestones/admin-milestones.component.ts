...
@Component({
  ...
  styleUrls: ['./admin-milestones.component.css']
})
export class AdminMilestonesComponent implements OnInit {
  ...
  datatableInit(){
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.adminhttp.PostAPI('admin/GetMilestones',dataTablesParameters).then((resdata: any) => {
  ...
  change_status(milestonesId) {
    var data = { milestonesId: milestonesId }
    this.adminhttp.PostAPI('admin/MilestonesChangeStatus', data).then((resdata: any) => {
      if (resdata.status == 200) {
