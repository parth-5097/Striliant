...
export class AdminBrandsComponent implements OnInit {
  ...
  datatableInit(){
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.adminhttp.PostAPI('admin/GetBrands',dataTablesParameters).then((resdata: any) => {
  ...
  change_status(brandId) {
    var data = { brandId: brandId }
    this.adminhttp.PostAPI('admin/BrandsChangeStatus', data).then((resdata: any) => {
      if (resdata.status == 200) {
  ...
  delete_brands(data) {
    Swal.fire({
      ...
      if (result.value) {
        this.adminhttp.PostAPI('admin/removeBrands', data).then((resdata: any) => {
          if (resdata.status == 200) {
            this.toastr.success(resdata.message);
            this.ReloadDatatable()
          } else {
