...
@Component({
  ...
  styleUrls: ['./admin-chat-list.component.css']
})
export class AdminChatListComponent implements OnInit {
  ...
  image_type = ['image/gif','image/jpeg','image/jpg','image/pjpeg','image/x-png','image/png']
  video_type = ['video/mp4','video/ogg','video/webm']
  audio_type = ['audio/ogg','audio/mpeg']
  document_type  = ["application/pdf", "application/doc", "application/docx", "application/rtf", "application/txt", "application/odf", "application/msword"]

  search_keyword = ''
  searchList = ''

  constructor(private chatService: ChatService, public router: Router, public toastr: ToastrService, public adminhttp: AdminHttpService, private formBuilder: FormBuilder,private route: ActivatedRoute, private commonService: CommonService) {
    if(localStorage.getItem('Admin')){
      this.loginUser = JSON.parse(localStorage.getItem('Admin'));
    }
  }

  ngOnInit(): void {
    this.unread_count = 0;
    this.chatService.connectUser({user_id : this.loginUser.adminId});
  ...
  searchFunction(event){
    if(event.target.value != ''){
      ...
      this.adminhttp.PostAPI('admin/get_data_search', data).then((resdata: any) => {
  ...
  getChatList(newuser='') {
    this.commonService.PostAPI('admin/chatList', { user_id : this.loginUser.adminId, newuser : newuser}).then((res) => this.onSuccess(res));
  }
  ...
  loadConversation(conversation_id, new_load)
  {
    if(this.searchList.length > 0){
      for(var i=0;i<this.searchList.length;i++){
        let search_conversation_id = 'personal_chat_' + this.searchList[i]['userId']
        if(this.chatList.length > 0){
          let foundIndex1 = this.chatList.findIndex(x => x.conversation_id === search_conversation_id);
          if(foundIndex1 == -1){
            if(search_conversation_id == conversation_id){
              let item = {
                'attachments' : [],
                'conversation_id' : 'personal_chat_' + this.searchList[i]['userId'],
                'display_name' : '',
                'message_content' : '',
                'message_id' : '',
                'message_status' : '',
                'recipient_id' : this.searchList[i]['userId'],
                'unread_message' : 0,
                'user_data' : {
                  'firstname' : this.searchList[i]['firstname'],
                  'lastname' : this.searchList[i]['lastname'],
                  'user_id_o' : this.searchList[i]['userId'],
                  'is_online' : this.searchList[i]['is_online'],
                  'company_name' : this.searchList[i]['company_name'],
                  'country' : this.searchList[i]['country'],
                  'designation' : this.searchList[i]['designation'],
                  'mobile' : this.searchList[i]['mobile'],
                  'phone' : this.searchList[i]['phone'],
                }
              }
              this.chatList.push(item)
              this.search_keyword = ''
    ...
    if (new_load)
    {
      ...
      const foundIndex = this.chatList.findIndex(x => x.conversation_id === conversation_id);
      this.chatUser = this.chatList[foundIndex].user_data;
    }
    this.commonService.PostAPI('admin/loadConversation', {
      conversation_id : conversation_id,
      user_id : this.loginUser.adminId,
      ...
      if (resdata.status)
      {
        ...
        resdata.data.forEach(item => {
          if(item.sender_user_id!=this.loginUser.adminId && item.message_status!=3)
          {
            item.receiver = this.loginUser.adminId;
            this.chatService.seenMessage(item);

            const foundIndex2 = this.chatList.findIndex(x => x.conversation_id === conversation_id);
            if(this.chatList[foundIndex2].unread_message>0) {
              // console.log( this.chatList[foundIndex2].unread_message);
              this.chatList[foundIndex2].unread_message = this.chatList[foundIndex2].unread_message-1;
            }
  ...
  keyFunc(event)
  {
    $('.input-group').removeClass('custom_error');
  }
  ...
  sendMessage(message_text)
  {
    ...
    this.chatService.sendMessage({
      ...
      from_user_id : this.loginUser.adminId,
  ...
  chatListListener(): void {
    this.chatService.chatListListener().subscribe((socketResponse: Message) =>
    {
      ...
      {
        const foundIndex = this.chatList.findIndex(x => x.conversation_id === chat_data.conversation_id);

        if(chat_data.last_message_send_by!=undefined)
        {
          this.chatList[foundIndex].last_message_send_by = chat_data.last_message_send_by;
        }
        ...
        if(chat_data.user_data!=undefined)
        {
          ...
          {
            this.chatList[foundIndex].user_data.is_online = chat_data.user_data.is_online;
          }
        }
  ...
  messageListener(): void {
    this.chatService.messageListener().subscribe((socketResponse) =>
    {
      ...
      if (socketResponse.event == 'add')
      {
        ...
        if(this.conversation_id == message_data.conversation_id  && this.loginUser.adminId!=message_data.sender_user_id)
        {
          message_data.receiver = this.loginUser.adminId;
        ...
        else if(this.loginUser.adminId!=message_data.sender_user_id)
        {
          message_data.receiver = this.loginUser.adminId;
      ...
      {
        const foundIndex = this.conversationMessageList.findIndex(x => x.message_id === message_data.message_id);

        if(message_data.message_status!=undefined)
        {
          this.conversationMessageList[foundIndex].message_status = message_data.message_status;
        }
