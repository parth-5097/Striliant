...
export class AdminBlogComponent implements OnInit {
  ...
  datatableInit(){
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.adminhttp.PostAPI('admin/GetBlog',dataTablesParameters).then((resdata: any) => {
  ...
  change_status(blogId) {
    var data = { blogId: blogId }
    this.adminhttp.PostAPI('admin/BlogChangeStatus', data).then((resdata: any) => {
      if (resdata.status == 200) {
  ...
  delete_blog(data) {
    Swal.fire({
      ...
      if (result.value) {
        this.adminhttp.PostAPI('admin/removeBlog', data).then((resdata: any) => {
          if (resdata.status == 200) {
            this.toastr.success(resdata.message);
            this.ReloadDatatable()
          } else {
