...
export class ForgotPasswordComponent implements OnInit, OnDestroy {
  ...
  constructor(public router: Router, public toastr: ToastrService, public http: HttpService, private formBuilder: FormBuilder,@Inject(DOCUMENT) private document: Document) { }
  ...
  onSubmitForgotForm() {
    ...
    this.http.PostAPI('users/forgot-password', data).then((resdata: any) => {
  ...
  onSubmitVerifyCheck(){
    ...
    this.http.PostAPI('users/verify-otp', data).then((resdata: any) => {
      if (resdata.status == 200) {
  ...
  onSubmitPasswordForm(){
    ...
    this.http.PostAPI('users/reset-password', data).then((resdata: any) => {
      this.verify_form_open = true
      if (resdata.status == 200) {
        this.toastr.success(resdata.message)
